...
app.post('/signup',async(req,res)=>{
    ...
    if (check) {
        return res.status(400).json({success:false,errors:"existing this email address"})
    }
    let cart = {};
    for (let i = 0; i < 300; i++) {
        cart[i]=0;
    ...
    const token = jwt.sign(data,'secret_fdeer');
...
// creating middleware to fetch user
    const fetchUser = async (req,res,next)=>{
        ...
        if (!token){
            ...
            try {
                const data = jwt.verify(token,'secret_fdeer');
                req.user = data.user;
                next();
            } catch (error) {
                res.status(401).send({errors:"Please authenticate using valid token"})
